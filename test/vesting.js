...
contract('Vesting', accounts => {
  ...
  it('should vest full amount after vesting period', async () => {
    ...
    // verify amount
    const receivedAmount = BigNumber(await token.balanceOf(acc1)).minus(acc1BeforeBalance)
    const expectedReceiveAmount = BigNumber(vestAmount)
    assert(receivedAmount.eq(expectedReceiveAmount), 'fully vested amount incorrect')
  })
  ...
  it('should vest linear partial amount during vesting period', async () => {
    ...
    // verify amount
    const receivedAmount = BigNumber(await token.balanceOf(acc1)).minus(acc1BeforeBalance)
    const expectedReceiveAmount = BigNumber(vestAmount).times(3 * DAY).div(7 * DAY).integerValue()
    assert(receivedAmount.eq(expectedReceiveAmount), 'partially vested amount incorrect')
  })
  ...
  it('should vest linear partial amount during vesting period only once', async () => {
    ...
    // verify amount
    const actualTimePassed = BigNumber(await latestBlockTimestamp() - 1).minus(vestTimestamp)
    const receivedAmount = BigNumber(await token.balanceOf(acc1)).minus(acc1BeforeBalance)
    const expectedReceiveAmount = BigNumber(vestAmount).times(actualTimePassed).div(7 * DAY).integerValue()
    assert(receivedAmount.lte(expectedReceiveAmount), 'partially vested amount incorrect')
  })
