...
contract('ZeroCouponBond', accounts => {
  ...
  it('mint zero coupon bond', async () => {
    ...
    assert(epsilonEq(await fractionalDeposit.balanceOf(zcbAddress), fractionalDepositBalance), 'fractional deposit not credited to zero coupon bonds contract')
  })

  it('redeem stablecoin from zero coupon bond', async () => {
    // create fractional deposit
    await depositNFT.setApprovalForAll(fractionalDepositFactory.address, true, { from: acc0 })
    const receipt = await fractionalDepositFactory.createFractionalDeposit(
      dInterestPool.address,
